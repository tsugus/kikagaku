...
\begin{dfn}[平面]
  ...
  となる実数 \(\lambda,\mu\) が存在する点 \(X\) すべての集合を、点 \(A,B,C\) を通る\textbf{平面}\index{へいめん@平面}という。
  ただし、公理 \ref{251101133903} によって高々一つしかないので普通は単に平面という。
\end{dfn}

\begin{dfn}[半平面、へり]
  一直線上にない任意の三点 \(A,B,C\) に対して、
  \[\overrightarrow{AX}=\lambda\overrightarrow{AB}+\mu\overrightarrow{AC},\quad \mu\geqq0\]
  となる実数 \(\lambda,\mu\) が存在する点 \(X\) すべての集合を、直線 \(AB\) を\textbf{へり}\index{へり@へり}とし点 \(C\) を含む\textbf{半平面}\index{はんへいめん@半平面}という。
  また、
...
\begin{dfn}[角、辺、頂点]
  ...
  \[\angle AOB\]
  と書かれる。
  半直線 \(OA\) と半直線 \(OB\) は、それぞれ \(\angle AOB\) の\textbf{辺}\index{へん@(角の)辺}とよばれ、点 \(O\) は \(\angle AOB\) の\textbf{頂点}\index{ちょうてん@(角の)頂点}とよばれる。\footnote{私たちの体系では、平角は半平面であり、実体として頂点の情報を含んでいない。
  同様に、周角は平面であり、辺の情報を含んでいない。
  つまり、記法との間にある種の乖離があるから、点の写像を考えるときに注意が必要になる場合がある。}
  二つの辺が半直線 \(h,k\) である角を
  \[\angle hk\]
  と表す。\footnote{この、二つの辺だけが特定できる表記で角を示す曖昧さを、新たに設定した慣習的ルールによって払拭することは可能である。
  つまり、\(\angle hk\) と \(\angle kh\) は同じ二つの辺で規定される二つの角をそれぞれ表す、とすればよい。
  \(\angle hk\) が凸角なら、\(\angle kh\) は凹角である。表記と角の「紐付け」を変える場合は、凸角 \(kh\) のように、もう一度明示的に角の種別を指定し直すこととする。
  そして、指定がない場合は凸角を意味しているものとする。
  \(\angle AOB\) のような点による角の指定の場合も同様である。}
\end{dfn}
...
\begin{dfn}[距離]
  任意の二点 \(P,Q\) に対し、二点間の\textbf{距離}\index{きょり@(二点間の)距離} \(d(P,Q)\) を
