...
\begin{dfn}[角、辺、頂点]
  ...
  \[\angle AOB\]
  と書かれる。
  半直線 \(OA\) と半直線 \(OB\) は、それぞれ \(\angle AOB\) の\textbf{辺}\index{へん@(角の)辺}とよばれ、点 \(O\) は \(\angle AOB\) の\textbf{頂点}\index{ちょうてん@(角の)頂点}とよばれる。\footnote{この体系では、平角は半平面であり、実体として頂点の情報を含んでいない。
