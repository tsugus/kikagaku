...
\begin{dfn}[\(\bm{E}\)]
  点というものすべての集合を
  \[\bm{E}\]
  と書く。\footnote{\(\bm{E}\) の元を \(A,B,C,\ldots\) のようにラテン文字の大文字で表し、\(E\) の部分集合を断りなくしばしばラテン文字の小文字で表す。}
\end{dfn}

\begin{dfn}[\(\bm{V}\)]
  ベクトルというものすべての集合を
  \[\bm{V}\]
  と書く。\footnote{\(\bm{V}\) の元を \(\vec{a},\vec{b},\vec{c},\ldots\) のようにラテン文字の小文字の上に右向きの矢印をつけて表す。}
\end{dfn}

\begin{dfn}[スカラー]
  実数のことをベクトルと区別して\textbf{スカラー}\index{すからー@スカラー}という。\footnote{スカラーを \(\alpha,\beta,\gamma,\ldots\) のようにギリシャ文字の小文字で表す。}
\end{dfn}

\begin{dfn}[倍]
  任意のスカラー \(\lambda\) と任意のベクトル \(\vec{a}\) に対して、それらの積 \(\lambda\vec{a}\)（公理 \ref{251101133701} によってただ一つ存在することが保証される）を、\(\vec{a}\) の \(\lambda\) \textbf{倍}\index{ばい@倍}ともいう。
\end{dfn}
...
\begin{dfn}[直線]
  異なる任意の二点 \(A,B\) に対して、（公理 \ref{251101133802} の記法の下で）
...
\begin{dfn}[半直線、端点]
  ...
  となる実数 \(\lambda\) が存在する点 \(X\) すべての集合を、点 \(A\) を\textbf{端点}\index{たんてん@(半直線の)端点}とし点 \(B\) を通る\textbf{半直線}\index{はんちょくせん@半直線}といい、
  \[\text{半直線} AB\]
   と表す。\footnote{この表記法は点の順番によって端点を示すが、絶対的な決まりではない。}
\end{dfn}

\begin{dfn}[平面]
  一直線上にない任意の三点 \(A,B,C\) に対して、
  \[\overrightarrow{AX}=\lambda\overrightarrow{AB}+\mu\overrightarrow{AC}\]
  となる実数 \(\lambda,\mu\) が存在する点 \(X\) すべての集合を、点 \(A,B,C\) を通る\textbf{平面}\index{へいめん@平面}という。ただし、公理 \ref{251101133903} によって高々一つしかないので普通は単に平面という。
\end{dfn}

\begin{dfn}[半平面、へり]
  一直線上にない任意の三点 \(A,B,C\) に対して、
  \[\overrightarrow{AX}=\lambda\overrightarrow{AB}+\mu\overrightarrow{AC},\quad \mu\geqq0\]
  となる実数 \(\lambda,\mu\) が存在する点 \(X\) すべての集合を、直線 \(AB\) を\textbf{へり}\index{へり@へり}とし点 \(C\) を含む\textbf{半平面}\index{はんへいめん@半平面}という。\footnote{注意点として、「点 \(C\) を含む半平面」の「含む」は述語ではなく単に名称の一部だということである。
  よって、\(C\) がへりの点である場合、実際に含まれる点にもかかわらず正しく指示機能を果たさない。
  しかし、この一種の曖昧さは論理的破綻をもたらすようなものではない。
  なぜならば、「へりの点を含む半平面」という指定の仕方が機能しないのであるが、これはむしろ命題としては限りなく正しいものだからである。}また、
...
\begin{dfn}[角、辺、頂点]
  ...
  \[\angle AOB\]
  と書かれる。
  半直線 \(OA\) と半直線 \(OB\) は、それぞれ \(\angle AOB\) の\textbf{辺}\index{へん@(角の)辺}とよばれ、点 \(O\) は \(\angle AOB\) の\textbf{頂点}\index{ちょうてん@(角の)頂点}とよばれる。
  二つの辺が半直線 \(h,k\) である角を
  \[\angle hk\]
  と表す。\footnote{任意ではない角が凸角とも凹角とも解釈できる場合は、慣習的に凸角を表すものとする。
  なお、この、二つの辺だけが特定できる表記で角を示す曖昧さを、新たに設定した慣習的ルールによって払拭することは可能である。
  つまり、\(\angle hk\) と \(\angle kh\) は同じ二つの辺で規定される二つの角をそれぞれ表す、とすればよい。
  \(\angle hk\) が凸角なら、\(\angle kh\) は凹角である。
  表記と角の「紐付け」を変える場合は、凸角 \(kh\) のように、もう一度明示的に角の種別を指定し直すこととする。}
\end{dfn}
...
\begin{axm}\label{251101143101}
  少なくとも一つの点が存在する。
\end{axm}

\begin{axm}\label{251101133802}
...
\begin{axm}\label{251101142901}
  少なくとも二つのベクトルが存在し、一方は他方のスカラー倍でない。
\end{axm}

\begin{axm}\label{251101133903}
...
\begin{axm}
  角の辺を \(h,k\) とするとき、\(\angle hk\) には、\textbf{角の大きさ}\index{かくのおおきさ@角の大きさ}とよばれるただ一つの実数
  \[\overline{\angle hk}\]
  が対応する。
\end{axm}

\begin{axm}\label{251101145101}
  角の辺を \(h,k\) とするとき、
  \(\overline{\angle hk}=0\) ならば \(h=k\) である。
\end{axm}

\begin{axm}\label{251101145201}
  \(\angle hk\) と \(\angle kl\) が辺以外に共通部分を持たず、ともに \(\angle hl\) の部分集合であるならば、
  \[\overline{\angle hl}=\overline{\angle hk}+\overline{\angle kl}\]
  が成り立つ。
\end{axm}

\begin{axm}\label{251101144301}
  二つの任意の凸角 \(AOB\) と凸角 \(A'O'B'\) に対して、
  \begin{gather*}
    d(O,A)=d(O',A'),\quad d(O,B)=d(O',B'), \\
    d(A,B)=d(A',B')
  \end{gather*}
  ならば
